...
(*        {rv : RandomVariable dom borel_sa rv_X} *)
  ...
  Definition Rbar_ge_dec (a b : Rbar) := Rbar_le_dec b a.
  ...
  Definition simple_approx (X:Ts->Rbar) (n:nat) : Ts -> R
    := fun ω : Ts =>
         ...
         if Rbar_ge_dec Xw (INR n) then (INR n) else
           match find (fun (start:R) => if Rbar_ge_dec Xw start then true else false) 
  ...
  Definition Rbar_ge (a b : Rbar) := Rbar_le b a.

  Lemma Rbar_not_ge_lt (r1 r2 : Rbar) :
    ~ (Rbar_ge r1 r2) -> Rbar_lt r1 r2.
  Proof.
    unfold Rbar_ge, Rbar_lt, Rbar_le; intros.
    destruct r1; destruct r2; try lra; tauto.
  Qed.
  ...
  Proof.
    ...
    - split.
      {
      generalize (Rbar_not_ge_lt _ _ n0); intros.
  ...
  Lemma Rbar_R_mult_assoc (a : Rbar) (b c : R) :
    Rbar_mult (Rbar_mult a b) c = Rbar_mult a (Rbar_mult b c).
  Proof.
    Admitted.

  Lemma Rbar_mult_1_r (a : Rbar) :
    Rbar_mult a 1 = a.
  Proof.
    destruct a.
    - simpl.
  ...
  Lemma Rbar_le_div_l (a b : Rbar) ( c : R) :
    0 < c ->
    (Rbar_le (Rbar_div a c) b) <-> 
    (Rbar_le a (Rbar_mult b c)).
  Proof.
    intros.
    unfold Rbar_div.
    rewrite Rbar_mult_pos_le with (z := mkposreal _  H).
    rewrite Rbar_mult_mult_pos.
    rewrite Rbar_mult_mult_pos.
    simpl.
    rewrite Rbar_R_mult_assoc.
    simpl.
    rewrite Rinv_l.
  ...
  Lemma Rbar_lt_div_r (a b : Rbar) (c : R) :
    0 < c ->
    (Rbar_lt a (Rbar_div b c)) <-> 
    (Rbar_lt (Rbar_mult a c) b).
  Proof.
    intros.
    unfold Rbar_div.
    rewrite Rbar_mult_pos_lt with (z := mkposreal _  H).    
    rewrite Rbar_mult_mult_pos.
    rewrite Rbar_mult_mult_pos.
    simpl.
    rewrite Rbar_R_mult_assoc.
  ...
  Proof.
    ...
    unfold simple_approx.
    match_destr; [
      case_eq (X omega); intros; rewrite H in Xlt; rewrite H in r; 
        simpl in Xlt; simpl in r; [lra | tauto| tauto] | ].
    clear n0.
    assert (pos1:(n * 2 ^ n > 0)%nat).
    {
      ...
      - destruct n; try lia.
        simpl in Xlt.
        specialize (posX omega).
        case_eq (X omega); intros; rewrite H in Xlt; rewrite H in posX;
          simpl in Xlt; simpl in posX; [lra | tauto | tauto].
      - simpl.
    ...
    - destruct (find_some_break _ _ _ H) as [l1 [l2 [eqq1 Fl1]]].
      ...
      + generalize (f_equal (fun x => nth (length d) x 0)%nat); intros HH.
        ...
        * destruct H as [le1 lt2].
          replace (Finite (INR (length d) / 2 ^ n)) with
               (Rbar_div (INR (length d)) ( 2 ^ n)) in r0.
          rewrite Rbar_le_div_l in r0.
          {
              ...
              specialize (HH _ _ eqq2).
              {
                ...
                - rewrite app_nth2 in HH.
                  + rewrite app_length in HH.
                    ...
                    match_destr_in Fl1.
                    apply n0.
                    replace (Finite (INR k / 2^n)) with (Rbar_div (INR k) (2^n)).
                    now apply  Rbar_le_div_l
                    ; [ apply pow2_pos |].
                    now simpl.
                  + rewrite app_length; simpl.
                    lia.
                - apply INR_lt.
                  replace (INR k < INR (n * 2 ^ n)) with
                      (Rbar_lt (INR k) (INR (n * 2 ^ n))).
            ...
            - assert (le2:(S k <= length d)%nat) by lia.
              apply le_INR in le2.
              generalize (Rbar_le_lt_trans _ _ _ r0 lt2); intros.
              red in H.
              lra.
          }            
          apply pow2_pos.
  ...
  Proof.
    ...
    - cut_to HH1.
      + destruct HH1 as [le1 lt1].
        split; intros.
        * replace (Finite (INR k / 2 ^ n)) with (Rbar_div (INR k) (2^n)) by (now simpl).
          apply  Rbar_le_div_l; [ apply pow2_pos |]; trivial.
        * replace (Finite (INR (S k) / 2 ^ n)) with (Rbar_div (INR (S k)) (2^n)) by (now simpl).
        ...
        * destruct (find_some_break _ _ _ eqq1) as [l1 [l2 [eqq2 Fl1]]].
          ...
          { 
              ...
              specialize (HH _ _ eqq2).
              {
                ...
                - rewrite app_length in HH.
                  ...
                  + rewrite <- HH in i.
                    ...
                    generalize (Rbar_le_lt_trans _ _ _ le1 n1); intros.
                    simpl in H.
                    lra.
                  + rewrite e in HH.
            ...
            - assert (le2:(S k <= length (rev l2))%nat) by lia.
              apply le_INR in le2.
              replace (Finite (INR (S k) / 2 ^ n)) with (Rbar_div (INR (S k)) (2 ^ n)) in lt2 by now simpl.
              apply Rbar_lt_div_r in lt2
              ; [| apply pow_lt; lra].
              replace (Finite  (INR (length (rev l2)) / 2 ^ n) ) with
                   (Rbar_div (INR (length (rev l2))) (2 ^ n)) in r by now simpl.
              apply Rbar_le_div_l in r
              ; [| apply pow_lt; lra].
              generalize (Rbar_le_lt_trans _ _ _ r lt2); intros.
              red in H.
              lra.
          }            
  ...
  Instance simple_approx_posrv (X:Ts->Rbar) (n:nat) : PositiveRandomVariable (simple_approx X n).
  Proof.
  ...
  Lemma simple_approx_range_event (X : Ts -> Rbar) (n:nat) (r : Rbar) :
    let rvals :=  filter (fun (z:Rbar) => if Rbar_le_dec z r then true else false)
                         (map (fun x : nat => Finite(INR x / 2 ^ n))
                              (seq 0 (S (n * 2 ^ n)))) in
    pre_event_equiv (fun omega : Ts => Rbar_le (simple_approx X n omega) r)
                    (pre_list_union (map (fun (z:Rbar) => (fun omega => (Finite (simple_approx X n omega)) = z))
                                 rvals)).
  Proof.
    ...
    unfold list_union.
    split; intros.
    - exists (fun omega => (Finite (simple_approx X n omega)) = (Finite (INR k / 2^n))).
      split.
      + rewrite in_map_iff.
        exists (Finite (INR k / 2^n)).
        split; trivial.
        rewrite filter_In.
        split.
        * rewrite in_map_iff.
  ...
  Proof.
    ...
    rewrite H0.
    assert (pre_event_equiv (fun z : Rbar => (Rbar_le (INR k / 2 ^ n) z) /\ 
                                          (Rbar_lt z (INR (S k) / 2 ^ n)))
  ...
  Instance simple_approx_rv (X : Ts -> Rbar)
    ...
    : forall (n:nat), RandomVariable dom Rbar_borel_sa (simple_approx X n).
  Proof.
    unfold RandomVariable.
    intros.
    apply Rbar_borel_sa_preimage; trivial.
    intros.
    generalize (simple_approx_vals X n); intros.
    ...
    - assert (pre_event_equiv
                (fun omega : Ts => Finite(simple_approx X n omega) = Finite(INR k / 2 ^ n))
                (fun omega : Ts => simple_approx X n omega = INR k / 2 ^ n)).
      {
        intros x.
        split; intros.
    ...
    - subst.
      ...
      + assert (pre_event_equiv
                  (fun omega : Ts => Finite(simple_approx X n omega) = Finite(INR n))
                  (fun omega : Ts => simple_approx X n omega = INR n)).
        {
          intros x.
          split; intros.
          - now apply Rbar_finite_eq in H3.
          - now apply Rbar_finite_eq.
        }
        rewrite H3; trivial.
  ...
  Proof.
    ...
    - cut_to IHk.
      + generalize (simple_approx_preimage_fin X n posX omega H); intros.
        ...
        * specialize (H3 H1).
          destruct H1.
          generalize (Rbar_le_lt_trans _ _ _ H0 H4); intros.
          red in H5.
          lra.
  ...
  Lemma Rbar_plus_lt_compat_l (a b : Rbar) (c : R) :
    Rbar_lt a b -> Rbar_lt (Rbar_plus c a) (Rbar_plus c b).
  Proof.
    destruct a; destruct b; simpl; try lra; try easy.
  Qed.
  ...
  Proof.
    ...
    unfold Rbar_minus.
    rewrite H1.
    replace  ((INR x + 1) / 2 ^ n) with ((INR x / 2 ^ n) + (/ 2 ^ n)) in H4 by lra.
    apply Rbar_plus_lt_compat_l with (c := - (INR x / 2^n)) in H4.
    simpl in H4.
    rewrite <- Rplus_assoc in H4.
    rewrite Rplus_opp_l in H4.
    rewrite Rplus_0_l in H4.
    simpl.
    rewrite Rbar_plus_comm in H4.
    apply H4.
  Qed.
  ...
  Global Instance pos_Rbar_pos (X : Ts -> R) 
  ...
  Global Instance borel_Rbar_borel (X : Ts -> R)
         (rv : RandomVariable dom borel_sa X) :
    RandomVariable dom Rbar_borel_sa X.
  Proof.
  Admitted.
  
  Lemma Rbar_borel_borel (X : Ts -> R)
