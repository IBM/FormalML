...
Section iVector.
  ...
  Fixpoint ivector A (n : nat) : Type :=
    match n with
    | 0 => unit
    | S m => prod A (ivector A m)
    end.
...
| (succ n), x, fz => _
  | (succ n), (x,xs), (fs k) => kth_projn xs *)

 Equations kth_projn {A} {n : nat} (v : ivector A n) (k : dfin (S n)) : A :=
    match n, v, k with
  | 0,x, _             => _
  | (S n), x, fz => _
  | (S n), (x,xs), (fs k) => _
  ...
  Fixpoint ivector_nth {A} (n k : nat) (v : ivector A n) : A :=
    match n with
      | 0 => _
