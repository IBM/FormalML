...
Lemma lemma2_beta (W : nat -> nat -> Ts -> R) (ω : Ts) 
 ...
 Qed.
  ...
  Lemma Y_alpha_beta_le (Y1 Y2 α beta : nat -> Ts -> R) (β : R) (D : Ts -> R) :
    (forall ω,  β * D ω >= 0) ->
    (rv_eq (Y1 0%nat) D) ->
    (rv_eq (Y2 0%nat) D) ->    
    (forall t ω, 0 <= α t ω <= 1) ->
  ...
  Proof.
   ...
   generalize (Y_alpha_beta_le Y Y' α beta β D); intros.
   cut_to H7; trivial.
...
*)
  ...
  Lemma lemma3_Jaakkola {n} (W : forall (i : nat),  (i < (S n))%nat -> nat -> nat -> Ts -> R) (ω : Ts) (ε G0 :R)
        (t0 : nat) (α x ww : nat -> Ts -> vector R (S n)) (M G : nat -> Ts -> R) 
    ...
    (forall t i pf,
        ...
        (vecrvnth i pf (α (t + t0)%nat) ω) * (vecrvnth i pf (ww (t + t0)%nat) ω)) ->
    (forall k, rv_eq (x (S k)) 
                     (vecrvplus (x k) (vecrvmult (α k) (vecrvplus (vecrvminus (XF k) (x k) ) (vecrvscalerv (G k) (ww k)))))) ->
...
(*    (almost prts (fun ω => forall t i pf, Rabs (W i pf t t0 ω) <= ε)) ->  *)
   ...
   Lemma lemma3_almost_Jaakkola {n} (W : forall (i : nat),  (i < (S n))%nat -> nat -> nat -> Ts -> R) (ε G0 :R)
         (α x ww : nat -> Ts -> vector R (S n)) (M G : nat -> Ts -> R) 
        (XF : nat -> Ts -> vector R (S n)) :
(*    (forall ω, M t0 ω <= G t0 ω) -> *)
    (forall ω t, M (S t) ω <= (1 + ε) * G t ω -> G (S t) ω = G t ω) ->
...
(*    (almost prts (fun ω => forall t i pf, Rabs (W i pf t t0 ω) <= ε)) ->  *)
   ...
   Proof.
     ...
     {
       intros.
       generalize (lemma3_Jaakkola W x0 ε G0 t0 α x ww M G XF); intros.
       cut_to H11; try easy.
       - now specialize (H11 t i pf).
       - apply H.
     }
...
*)
    ...
    (forall k, rv_eq (X (S k)) 
                     (vecrvplus (X k) (vecrvmult (α k) (vecrvplus (vecrvminus (XF k) (X k) ) (w k))))) ->
    exists (D0 : Ts -> R),  forall k, almostR2 prts Rle (rvmaxabs (X k)) D0.
  Proof.
    intros.
    assert (exists γ,β < γ < 1).
    ...
    assert (adaptX : IsAdapted (Rvector_borel_sa (S n)) X F).
    {
      ...
      - easy.
      - assert (RandomVariable (F (S n0)) (Rvector_borel_sa (S n))
                               (vecrvplus (X n0) (vecrvmult (α n0) (vecrvplus (vecrvminus (XF n0) (X n0)) (w n0))))).
        {
          apply Rvector_plus_rv.
          - now apply (RandomVariable_sa_sub (isfilt n0)).
          - apply Rvector_mult_rv.
    ...
    assert (almost prts (fun ω =>
                           ...
                           exists t0 : nat, 
                             forall t,
                               (G t0 ω = G (t + t0)%nat ω))).
    {
      generalize (lemma3_almost_Jaakkola WW ε G0 α X ww M G XF); intros.
      cut_to H21; try easy.
      - revert H21.
        apply almost_impl, all_almost; intros ???.
        destruct (H21 H22).
...
*)
  ...
  Theorem Tsitsiklis3_beta_pos_Jaakkola {n}
    (X w α : nat -> Ts -> vector R n)
    (β : R) (D0' : Ts -> R) 
    (x' : vector R n)
...
*)
    ...
    (forall k, almostR2 prts Rle (rvmaxabs (X k)) D0') ->
    0 < β < 1 ->
    (forall k ω, Rvector_max_abs (Rvector_minus (XF k ω) x') <=
                 β * Rvector_max_abs (Rvector_minus (X k ω) x')) ->
    (forall k, rv_eq (X (S k)) 
                     (vecrvplus (X k) (vecrvmult (α k) (vecrvplus (vecrvminus (XF k) (X k) ) (w k))))) ->
    almost prts (fun ω => is_lim_seq (fun n => Rvector_max_abs (Rvector_minus (X n ω) x')) 0).
         ...
         - assert (forall ω, 0 < β * eps * (D k ω)).
           ...
           + intros.
             ...
             apply Rplus_ge_reg_l with
               (r := - vector_nth i pf x').
             ring_simplify.
             assert (Rabs (vector_nth i pf (Rvector_minus (XF (t + tauk x)%nat x)
                                                          x')) <=  β * D k x).
             {
               eapply Rle_trans.
               apply Rvector_max_abs_nth_le.
...
*)
 ...
 Theorem Tsitsiklis3_beta_0_Jaakkola {n}
   (X w α : nat -> Ts -> vector R n)
   (β : R) (D0' : Ts -> R)
   (x' : vector R n)
   (XF : nat -> Ts -> vector R n)
...
*)
  ...
  Proof.
    intros.
    pose (Xx := fun n ω => Rvector_minus (X n ω) x').
    assert (forall k ω, XF k ω = x').
    {
      intros.
      specialize (H6 k ω).
      rewrite H5 in H6.
      rewrite Rmult_0_l in H6.
      generalize (Rvector_max_abs_nonneg (Rvector_minus (XF k ω) x')); intros.      
      assert (Rvector_max_abs (Rvector_minus (XF k ω) x') = 0) by lra.
      apply Rvector_max_abs_zero in H9.
      apply (f_equal (fun v => Rvector_plus v x')) in H9.
      unfold Rvector_minus in H9.
      rewrite <- Rvector_plus_assoc in H9.
...
*)
  ...
  Theorem Tsitsiklis3_max_abs_Jaakkola {n} 
    (X w α : nat -> Ts -> vector R n) 
    (β : R) (D0 : Ts -> R)  
    (x' : vector R n)
    (XF : nat -> Ts -> vector R n)
...
*)
  ...
  Proof.
    intros.
    destruct (Rlt_dec 0 β).
    - now apply (@Tsitsiklis3_beta_pos_Jaakkola
                   _
                   _
                   w α β D0 _ XF _ _
                   filt_sub
                   ...
                   rvw).
    - assert (β = 0) by lra.
      now apply (@Tsitsiklis3_beta_0_Jaakkola _ _
                   w α β D0 _ XF _ _
                   filt_sub
                   _ _ _
                   posD0
...
*)
  ...
  Theorem Tsitsiklis3_Jaakkola {n} 
    (X w α : nat -> Ts -> vector R n) 
    (β : R) (D0 : Ts -> R)  
    (x' : vector R n)
    (XF : nat -> Ts -> vector R n)
...
*)
  ...
  Proof.
    intros.
    generalize (Tsitsiklis3_max_abs_Jaakkola X w α β D0 x' XF _ filt_sub); intros.
    specialize (H8 _ _ _ posD0 _ _ iscond).
    cut_to H8; try easy.
    revert H8; apply almost_impl, all_almost; intros ????.
    now apply lim_seq_maxabs.
...
*)
  ...
  Lemma Rvector_mult_plus_distr_l {n} (x y z : vector R n) :
    Rvector_mult x (Rvector_plus y z) = Rvector_plus (Rvector_mult x y) (Rvector_mult x z).
  Proof.
...
Proof.
  intros.
  pose (X' := fun n ω => Rvector_plus (X n ω) x').
  pose (XF' := fun n ω => Rvector_plus (XF n ω) x').
  pose (D0' := fun ω => D0 ω + Rvector_max_abs x').
  generalize (Tsitsiklis3_Jaakkola_alpha_beta X' w α beta β D0' XF' isfilt filt_sub adapt_alpha); intro Tsit3.
  specialize (Tsit3 adapt_beta).
  assert (rvX0' : RandomVariable (F 0%nat) (Rvector_borel_sa n) (X' 0%nat)).
  {
    unfold X'.
    apply Rvector_plus_rv; trivial.
    apply rvconst.
  }
  assert (XF'_rv :  forall k, RandomVariable (F (S k)) (Rvector_borel_sa n) (XF' k)).
  {
    intros.
    unfold XF'.
    apply Rvector_plus_rv; trivial.
  ...
  assert (posD0' : forall ω : Ts, 0 < D0' ω).
  {
    intros.
    unfold D0'.
    replace (0) with (0 + 0) by lra.
    apply Rplus_lt_le_compat; trivial.
    apply Rvector_max_abs_nonneg.
  }
  specialize (Tsit3 rvX0' XF'_rv posD0' adapt_w rvw iscond).
  cut_to Tsit3; trivial.
  cut_to Tsit3.
  - revert Tsit3.
    apply almost_impl.
    apply all_almost.
    ...
    specialize (H12 i pf).
    unfold X' in H12.
    apply is_lim_seq_ext with (v := (fun m => vector_nth i pf (X m x) - vector_nth i pf x')) in H12.
    + apply is_lim_seq_ext with (u := (fun m => (vector_nth i pf (X m x) - vector_nth i pf x') + vector_nth i pf x')).
      * intros.
        lra.
      * apply is_lim_seq_plus with (l1 := 0) (l2 := vector_nth i pf x'); trivial.
        -- apply is_lim_seq_const.
        -- unfold is_Rbar_plus.
           simpl.
           now rewrite Rplus_0_l.
    + intros.
      now rewrite Rvector_nth_minus.
   - intros.
     unfold X', XF'.
     apply H10.
   - intros.
   ...
   - intros.
     ...
     intros ??.
     unfold X', D0', rvmaxabs, Rvector_minus.
     eapply Rle_trans.
     apply Rvector_max_abs_triang.
...
*)
  ...
  Theorem Tsitsiklis_1_3_max_abs_Jaakkola {n} 
    (β : R) 
    (X w α : nat -> Ts -> vector R n)
    (x' : vector R n)
    (XF : nat -> Ts -> vector R n)
...
*)
  ...
  Proof.
    intros.
    destruct n; try lia.
    generalize (Tsitsiklis1_Jaakkola β X w α XF isfilt filt_sub 
                            adapt_alpha); intros Tsit1.
    specialize (Tsit1 adapt_w _ _ iscond H H0 H1 H2 H3 H4).
    assert (exists D : nonnegreal,
          forall k ω,
          Rvector_max_abs (XF k ω) <= β * Rvector_max_abs (X k ω) + D).
    {
      ...
      exists (mknonnegreal _ H7).
      intros.
      generalize (Rvector_max_abs_triang_inv (XF k ω) x'); intros.
      generalize (Rle_trans _ _ _ H8 (H5 k ω)); intros.
      simpl.
      unfold Rvector_minus in H9.
      generalize (Rvector_max_abs_triang (X k ω) (Rvector_opp x')); intros.
    ...
    destruct Tsit1 as [D0 Tsit1].
    pose (D0' := rvplus (const 1) (rvabs D0)).
    generalize (Tsitsiklis3_max_abs_Jaakkola X w α β D0' x' XF isfilt filt_sub); intros Tsit3.
    specialize (Tsit3 adapt_alpha _ _).
    assert  (forall ω : Ts, 0 < D0' ω).
    {
      unfold D0'.
...
*)
  ...
  Proof.
    intros.
    generalize (Tsitsiklis_1_3_max_abs_Jaakkola β X w α x' XF _ filt_sub); intros.
    specialize (H7 _ _  npos _ _ _ ).
    cut_to H7; try easy.
    revert H7; apply almost_impl, all_almost; intros ????.
    now apply lim_seq_maxabs.
