...
Proof.
 ...
 (* assert (nnf : NonnegativeFunction (cutoff_eps_rv n eps Sum))
  ...
  rewrite H1.
  generalize (Chebyshev_ineq_div_mean (fun w => Rabs(cutoff_eps_rv n eps Sum w)) H0 0 eps); intros H3.
  erewrite <- simple_NonnegExpectation in H3.  
  simpl in H3.
  assert (event_equiv 
            (event_ge dom (rvabs (rvminus (fun w => Rabs(cutoff_eps_rv n eps Sum w)) (const 0))) eps)
            (event_ge dom (fun w => Rabs(cutoff_eps_rv n eps Sum w)) eps)).
  {
    intro x.
  ...
  rewrite H in H3.
  assert (rv_eq (rvsqr (rvminus (fun w => Rabs(cutoff_eps_rv n eps Sum w)) (const 0)))
                (rvsqr (cutoff_eps_rv n eps Sum))).
  {
    intros x.
    unfold rvsqr.
    rewrite rvminus_unfold.
    unfold const. rewrite Rminus_0_r. now rewrite <-Rsqr_abs.
  }
