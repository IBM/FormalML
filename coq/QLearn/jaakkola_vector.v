...
(*
      - unfold IsAdapted; intros.
        unfold IsFiltration in isfilt.
        assert (sa_sub (F 0%nat) (F n)).
        {
          induction n.
          - apply sa_equiv_sub.
            reflexivity.
        ...
        apply (RandomVariable_sa_sub H19).
        apply H18.
*)
      - unfold IsAdapted; intros.
        apply vecrvnth_rv.
...
*)
  ...
  Proof.
    ...
    - destruct H10 as [? [??]].
      ...
      intros.
      unfold XX, XXF.
      specialize (H12 (k + xx)%nat i pf ω).
      eapply Rbar_le_trans; cycle 1.
      apply H12.
      simpl.
      right.
...
*)
  ...
  Proof.
    ...
    generalize (Finite_conditional_variance_alt_scale f c sub); intros.
    ...
    assert (isfe5 : IsFiniteExpectation prts
            (rvmult (FiniteConditionalExpectation prts sub f) f)) by admit.
    generalize (conditional_variance_alt f sub); intros.
    assert (rv2' : RandomVariable dom borel_sa
          (rvsqr
             (rvminus (rvscale c f)
    ...
    assert (isfe5' : IsFiniteExpectation prts
            (rvmult (FiniteConditionalExpectation prts sub (rvscale c f))
               (rvscale c f))) by admit.
    generalize (conditional_variance_alt (rvscale c f) sub); intros.
    revert H0; apply almost_impl.
    revert H1; apply almost_impl.
    revert H2; apply almost_impl.
    apply all_almost; intros ????.
...
Proof.
  ...
  specialize (jaak _ _ _ _ H H0 H1 H2 H3 H4 H5 H6).
  assert  (forall (k i : nat) (pf : (i < S N)%nat) (ω : Ts),
          Rabs
            (FiniteConditionalExpectation prts (filt_sub k) 
               (vecrvnth i pf (XF' k)) ω) <= γ * Rvector_max_abs (X' k ω)).
  {
    ...
    specialize (H7 k ω).
    assert (vector_IsFiniteExpectation prts (XF' k)).
    {
      now apply vector_nth_IsFiniteExpectation.
    }
    ...
    - eapply Rle_trans.
      ...
      f_equal.
      unfold XF'.
      generalize (pos_scaled_vec_fincondexp W (filt_sub k) (XF k)); intros.
      admit.
    - rewrite vector_FiniteConditionalExpectation_nth.
      now apply FiniteConditionalExpectation_ext.
  }
...
(*    
    generalize IsFiniteExpectation_proper_almostR2; intros.
         ...
         (rvsqr
            (rvminus (vecrvnth i pf (fun ω : Ts => pos_Rvector_mult (XF k ω) W))
               (FiniteConditionalExpectation prts (filt_sub k)
                  (vecrvnth i pf (fun ω : Ts => pos_Rvector_mult (XF k ω) W)))))); intros.

    apply IsFiniteExpectation_proper_almostR2.
    apply IsFiniteExpectation_proper.
    intros ?.
    unfold rvsqr, rvminus, vecrvnth, rvplus, rvopp, rvscale; simpl.
    assert (rv_eq
              ...
              (FiniteConditionalExpectation prts (filt_sub k)
                 (rvscale (vector_nth i pf W)
                          (fun omega => vector_nth i pf (XF k omega))))).
    {
      intros.
      apply FiniteConditionalExpectation_ext.
      intros ?.
      unfold pos_Rvector_mult, rvscale.
      rewrite Rvector_nth_mult, vector_nth_map.
      lra.
   }
   rewrite H12.
    generalize FiniteCondexp_scale; intros.
...
generalize isfe2; intros.
                                         
    assert (forall w,
               IsFinite
    apply IsFiniteExpectation_bounded with (rv_X1 := const 0) (rv_X3 := (rvsqr
               (rvminus (vecrvnth i pf (XF k))
                  (FiniteConditionalExpectation prts (filt_sub k)
                     (vecrvnth i pf (XF k)))))).
    admit.
...
*)
  ...
  - intros.
    ...
    assert (Rvector_max_abs ((vector_FiniteConditionalExpectation prts (filt_sub k) (XF' k)) ω) <= γ * Rvector_max_abs (X' k ω)).
    {
      admit.
    }
    eapply Rle_trans; cycle 1.
