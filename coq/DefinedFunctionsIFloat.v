...
Module A := TranscendentalFloatFast F.
  ...
  Definition float : Type := F.type.
  Definition prec : Z := 53%Z.

  Definition Fzero := F.zero.
  Definition Fopp (x : float) := F.neg x.
  Definition Fplus (x y : float) := F.add rnd_NE prec x y.
  Definition Fminus (x y : float) := F.sub rnd_NE prec x y.
  Definition Fmult (x y : float) := F.mul rnd_NE prec x y.
  Definition Fdiv (x y : float) := F.div rnd_NE prec x y.  
  Definition Fsqrt (x : float) := F.sqrt rnd_NE prec x.
  Definition Fabs (x : float) := F.abs x.
  
  Definition exp (x: float) := A.I.midpoint(A.exp_fast prec x).
  Definition ln (x: float) := A.I.midpoint(A.ln_fast prec x).
  Definition sin (x: float) := A.I.midpoint(A.sin_fast prec x).
  Definition cos (x: float) := A.I.midpoint(A.cos_fast prec x).

  Definition Z2F (i:Z) := F.fromZ i.
  Definition FPI := F.mul rnd_NE prec (Z2F 4) (A.I.midpoint (A.pi4 prec)).
...
(*
  ...
  Inductive Xcomparison : Set :=
    Xeq : Xcomparison | Xlt : Xcomparison | Xgt : Xcomparison | Xund : Xcomparison
